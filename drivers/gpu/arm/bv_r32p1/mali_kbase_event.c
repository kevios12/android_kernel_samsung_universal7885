...
{
	if (katom->core_req & BASE_JD_REQ_EXTERNAL_RESOURCES) {
		init_kthread_work(&katom->work, kbase_event_process_noreport_worker);
		queue_kthread_work(&kctx->kbdev->event_worker, &katom->work);
	} else {
		kbase_event_process(kctx, katom);
	}
}
...
void kbase_event_cleanup(struct kbase_context *kctx)
{
	...
	KBASE_DEBUG_ASSERT(kctx);
	flush_kthread_worker(&kctx->kbdev->event_worker);
