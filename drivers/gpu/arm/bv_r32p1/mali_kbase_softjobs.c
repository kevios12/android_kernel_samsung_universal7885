	...
	list_for_each_safe(entry, tmp, &kctx->waiting_soft_jobs) {
		...
		case BASE_JD_REQ_SOFT_EVENT_WAIT:
			if (katom->jc == evt) {
				...
				katom->event_code = BASE_JD_EVENT_DONE;
				init_kthread_work(&katom->work,
					  kbasep_soft_event_complete_job);
				queue_kthread_work(&kctx->kbdev->job_done_worker,
					   &katom->work);
			} else {
...
static void kbase_fence_debug_timeout(struct kbase_jd_atom *katom)
{
	...
	if (work) {
		...
		queue_kthread_work(&kctx->kbdev->job_done_worker, &work->work);
	}
}
...
void kbasep_soft_job_timeout_worker(struct timer_list *timer)
{
	...
	list_for_each_safe(entry, tmp, &kctx->waiting_soft_jobs) {
		...
		case BASE_JD_REQ_SOFT_EVENT_WAIT:
			...
			katom->event_code = BASE_JD_EVENT_JOB_CANCELLED;
			init_kthread_work(&katom->work, kbasep_soft_event_complete_job);
			queue_kthread_work(&kctx->kbdev->job_done_worker, &katom->work);
			break;
#ifdef CONFIG_MALI_FENCE_DEBUG
		case BASE_JD_REQ_SOFT_FENCE_WAIT:
...
void kbase_jit_retry_pending_alloc(struct kbase_context *kctx)
{
	...
	list_for_each_safe(i, tmp, &jit_pending_alloc_list) {
		...
		if (kbase_jit_allocate_process(pending_atom) == 0) {
			/* Atom has completed */
			init_kthread_work(&pending_atom->work,
					kbasep_jit_finish_worker);
			queue_kthread_work(&kctx->kbdev->job_done_worker, &pending_atom->work);
		}
