		...
		ktime_t *end_timestamp, kbasep_js_atom_done_code done_code)
{
	...
	init_kthread_work(&katom->work, kbase_jd_done_worker);
	queue_kthread_work(&kbdev->job_done_worker, &katom->work);
}
...
void kbase_jd_cancel(struct kbase_device *kbdev, struct kbase_jd_atom *katom)
{
	...
	init_kthread_work(&katom->work, jd_cancel_worker);
...
void kbase_jd_exit(struct kbase_context *kctx)
{
	...
	flush_kthread_worker(&kctx->kbdev->job_done_worker);
}
