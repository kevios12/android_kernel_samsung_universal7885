	...
	while (rear != front) {
		...
		if (id == acpm_reg_id) {
			...
			if (reg_id == NO_SS_RANGE)
				exynos_ss_regulator("outSc", val >> 12, (val >> 4) & 0xFF, val & 0xF);
			else if (reg_id == NO_SET_REGMAP)
				exynos_ss_regulator("noMap", val >> 12, (val >> 4) & 0xFF, val & 0xF);
			else
				exynos_ss_regulator(regulator_ss[reg_id].name, val >> 12,
