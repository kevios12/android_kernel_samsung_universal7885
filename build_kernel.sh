...
copy_oneui() {
	...
	if [[ "$codename" = "a40" || "$codename" == "a30s" || "$codename" == "a30" ]]; then
		...
		cp $ANYKERNEL/prebuilt/dtb_enf.img "$ANYKERNEL/oneui/enforce/dtb.img"
		cp $ANYKERNEL/prebuilt/dtb_perm.img "$ANYKERNEL/oneui/permissive/dtb.img"
...
copy_aosp() {
	...
	if [[ "$codename" = "a40" || "$codename" == "a30s" || "$codename" == "a30" ]]; then
		...
		cp $ANYKERNEL/prebuilt/dtb_enf.img "$ANYKERNEL/aosp/enforce/dtb.img"
		cp $ANYKERNEL/prebuilt/dtb_perm.img "$ANYKERNEL/aosp/permissive/dtb.img"
	elif [ "$codename" = "jackpotlte" ]; then
